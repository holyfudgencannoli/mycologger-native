...
export async function create(
  ...
  const result = await safeRun(db,
    "INSERT INTO sterilizations (notes, created_at) VALUES (?, ?)",
    [notes, created_at]
  );
...
export async function readAll(db: SQLiteDatabase) {
  return await safeSelectAll(db, "SELECT * FROM sterilizations ORDER BY id ASC");
}
...
export async function getById(
  ...
  return await safeSelectOne<{
    ...
    notes: string;
    created_at: number;
  }>(db, "SELECT * FROM sterilizations WHERE id = ?", [id]);
}
...
export async function destroy(db: SQLiteDatabase, id: number) {
  const result = await safeRun(
    db,
    "DELETE FROM sterilizations WHERE id = ?",
    [id]
  );
...
export async function exists(db: SQLiteDatabase, id: number) {
  const row = await safeSelectOne<{ count: number }>(
    db,
    "SELECT COUNT(*) as count FROM sterilizations WHERE id = ?",
    [id]
  );
