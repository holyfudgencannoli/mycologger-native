...
export async function create(
  ...
  const result = await safeRun(db,
    "INSERT INTO consumable_items (item_id, name, category, subcategory) VALUES (?, ?, ?, ?)",
...
export async function readAll<ConsumableItem>(db: SQLiteDatabase) {
  return await safeSelectAll<ConsumableItem>(db, "SELECT * FROM consumable_items ORDER BY id ASC");
}
...
export async function getById(
  ...
  return await safeSelectOne<{
    ...
    category: string;
    subcategory: string;
  }>(db, "SELECT * FROM consumable_items WHERE id = ?", [id]);
}
...
export async function getByName(
  ...
  return await safeSelectOne<{
    ...
    category: string;
    subcategory: string;
  }>(db, "SELECT * FROM consumable_items WHERE name = ?", [name]);
}
...
export async function destroy(db: SQLiteDatabase, id: number) {
  const result = await safeRun(
    db,
    "DELETE FROM consumable_items WHERE id = ?",
    [id]
  );
...
export async function exists(db: SQLiteDatabase, id: number) {
  const row = await safeSelectOne<{ count: number }>(
    db,
    "SELECT COUNT(*) as count FROM consumable_items WHERE id = ?",
    [id]
  );
