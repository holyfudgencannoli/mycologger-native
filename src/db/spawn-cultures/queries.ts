...
export async function create(
  ...
  const result = await safeRun(db,
    "INSERT INTO spawn_cultures (media_id, recipe_id, volume_amount, volume_unit, last_updated, sterilized_id, inoculated_id, germinated_id, colonized_id, contaminated_id, harvested_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
export async function readAll(db: SQLiteDatabase) {
  return await safeSelectAll(db, "SELECT * FROM spawn_cultures ORDER BY id ASC");
}
...
export async function getById(
  ...
  return await safeSelectOne<{
    ...
    media_id: number;
        ...
        contaminated_id: number;
        harvested_id: number;
  }>(db, "SELECT * FROM spawn_cultures WHERE id = ?", [id]);
}
