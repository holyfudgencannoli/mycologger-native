...
export default function BioMaterialInventory({ navigation, state }) {
  ...
  const getBioMatData = async () => {
    try {
      const bioMatData: ItemProps[] = await BioMat.getAllByType(db, 'bio_material');
      setBioMaterials(bioMatData);
      setInventoryLogs(bioMatData);
        ...
        >
          <Surface style={styles.surfaceMetaContainer}>
            <Surface style={styles.surfaceContainer}>
              <Form.Control name="table" label="Bio Materials" labelStyle={styles.label}>  
